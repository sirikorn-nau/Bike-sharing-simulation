...
def find_route_osm(road, start_latlon, end_latlon, algorithm, exclude_pos=None):
    ...
    if algorithm == 'a_star':
        ...
        try:
            ...
            route_nodes = nx.astar_path(road, start_node, end_node, weight='length') #nx.astar_path: เป็นฟังก์ชันใน networkx ที่ใช้ A*  , weight='length': กำหนดให้ A* ใช้ความยาวของเส้นทาง (edge weight) เป็นค่า cost ในการคำนวณเส้นทาง
...
def m_star_search(graph, start_positions, destination_positions, station_locations, t_per_meter, simulation_time_step, max_time_step, road):
    ...
    for agent_id in range(num_agents):
        ...
        osm_path_start = find_route_osm(road, start, start_station, 'm_star')
        ...
        osm_path_middle = find_route_osm(road, start_station, end_station, 'm_star')
        ...
        osm_path_end = find_route_osm(road, end_station, goal, 'm_star')
        if not osm_path_end:
