...
def create_summary_chart(agent_grid_steps_abs, agent_grid_steps_cbs, agent_grid_steps_mstar):
    ...
    ax.bar(x - width, agent_grid_steps_abs, width=width, label='A*', color='blue')
    ...
    ax.set_title('Comparison of Grid Steps: A* vs CBS vs M*')
    ax.set_xticks(x)
...
def show_summary_chart(agent_grid_steps_abs, agent_grid_steps_cbs, agent_grid_steps_mstar):
    st.write("### Summary Chart: A* vs CBS vs M*")
...
def show_summary_chart_plotly(agent_grid_steps_abs, agent_grid_steps_cbs, agent_grid_steps_mstar):
    ...
    fig = px.bar(df, x='Agent', y=['A*', 'CBS', 'M*'], barmode='group', title='Comparison of Grid Steps: A* vs CBS vs M*')
    st.plotly_chart(fig)
